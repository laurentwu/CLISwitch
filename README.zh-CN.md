...
CLISwitch 是一个基于 Tauri 2 的本地桌面应用，用于查看、保存并安全切换 Claude Code、Codex CLI、OpenCode 与 Qwen Code 的用户级配置。0.1 面向 Windows、macOS 和 Linux，不支持移动端。
...
- Qwen Code 支持经 v0.23 验证的 API Key/OpenAI Chat 配置，包括模型与地址精确选择，以及同路由账号安全切换。
- API Key 与 OAuth 原文均完整明文显示、复制和编辑；OAuth 原文允许保存任意 UTF-8 内容，即使格式已损坏。
- 按 CLI 查看文件预览、先保存再应用、摘要冲突检测、按 CLI 顺序执行、原子替换、写后验证、失败回滚、取消，以及只重试失败项。
...
`src-tauri/catalog/config-templates` 中的配置模板；模板必须按该目录记录的固定提交审查流程
单独更新，并随应用版本发布。
